...
class CoTXLayer(nn.Module):
    def __init__(self, dim, kernel_size, norm=None):
        ...
        self.key_embed = nn.Sequential(
            Conv2d(dim, dim, self.kernel_size, stride=1, padding=self.kernel_size//2, groups=8, bias=False, norm=get_norm(norm, dim)),
        ...
        self.embed = nn.Sequential(
            Conv2d(2*dim, dim//factor, 1, groups=self.dw_group, bias=False, norm=get_norm(norm, dim//factor)),
            nn.ReLU(inplace=True),
    ...
    def __init__(
        ...
        *,
        ...
        if stride_3x3 > 1:
            ...
            self.avd = None
        self.conv2 = CoTXLayer(bottleneck_channels, kernel_size=3, norm=norm) 
...
class CoTNeXt(Backbone):
    def __init__(self, stem, stages, num_classes=None, out_features=None):
        ...
        for i, blocks in enumerate(stages):      # fusing stages 
            assert len(blocks) > 0, len(blocks)
            for block in blocks:
                assert isinstance(block, CNNBlockBase), block

            name = "res" + str(i + 2)
            stage = nn.Sequential(*blocks)  # stack layers of a block sequentially
    ...
    def make_xstage(
        ...
        if first_stride is not None:
            ...
            logger.warning(
                "ResNet.make_xstage(first_stride=) is deprecated!  "
            ...
            for k, v in kwargs.items():
                if k.endswith("_per_block"):
                    assert len(v) == num_blocks, (
                        f"Argument '{k}' of make_xstage should have the "
                        f"same length as num_blocks={num_blocks}."
                    )
                    newk = k[: -len("_per_block")]
                    assert newk not in kwargs, f"Cannot call make_xstage with both {k} and {newk}!"
...
CoTNeXtBlockBase = CNNBlockBase

def make_xstage(*args, **kwargs):
    """
    Deprecated alias for backward compatibiltiy.
    """
    return CoTNeXt.make_xstage(*args, **kwargs)


@BACKBONE_REGISTRY.register()
def build_cotnext_backbone(cfg, input_shape):
    ...
    num_groups          = 4  #cfg.MODEL.RESNETS.NUM_GROUPS
    width_per_group     = 24 #cfg.MODEL.RESNETS.WIDTH_PER_GROUP
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        stage_kargs["dilation"] = dilation
        stage_kargs["num_groups"] = num_groups
        stage_kargs["block_class"] = CoTXBlock

        blocks = CoTNeXt.make_xstage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return CoTNeXt(stem, stages, out_features=out_features).freeze(freeze_at)
