...
CoTNetBlockBase = CNNBlockBase

def make_stage(*args, **kwargs):
    """
    Deprecated alias for backward compatibiltiy.
    """
    return CoTNet.make_stage(*args, **kwargs)


@BACKBONE_REGISTRY.register()
def build_cotnet_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        blocks = CoTNet.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return CoTNet(stem, stages, out_features=out_features).freeze(freeze_at)
